...
- (void)viewDidLoad
{
    ...
    UIButton *fuBtn = ({
        ...
        [button setImage:[UIImage imageNamed:@"camera_btn_filter_normal"] forState:UIControlStateNormal];
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(60, 60));
            make.bottom.equalTo(self.cameraPreviewView).with.offset(-220);
            make.left.equalTo(self.cameraPreviewView).offset(20);
        }];
        button;
    });
    [fuBtn addTarget:self action:@selector(showDemoBar:) forControlEvents:UIControlEventTouchUpInside];
